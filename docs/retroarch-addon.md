...
on the Pi's V3D GPU + CPU, **not** inside an emulated container (that was tried
and was far too laggy for real-time play). It reuses the same
H.264-over-WebSocket pipeline as the CarPlay path.
...
```
RetroArch tile → POST /retroarch/start (or /launch) → sudo tesla-pi-retroarch start
  → systemctl start cage-retroarch.service
      = cage -- /usr/local/bin/tesla-pi-retroarch-session
  ...
  → scripts/uinput-pad.py creates a /dev/uinput virtual gamepad; the browser
    overlay (and any BT pad paired to the Pi) drive it via /retroarch/input (WS)
      → RetroArch udev joypad driver, bound to RetroPad by tesla-pi-pad.cfg
```

RetroArch is **not kept warm** — it runs the core at
...
| `scripts/tesla-pi-retroarch-session` → `/usr/local/bin/` | cage's command: pins the mode, execs RetroArch |
| `scripts/tesla-pi-retroarch` → `/usr/local/sbin/` | privileged start/stop/status helper (sudoers NOPASSWD) |
...
| `conf/retroarch/autoconfig/tesla-pi-pad.cfg` | binds the virtual pad → RetroPad automatically |
| `conf/99-tesla-pi-uinput.rules` | grants group `input` access to `/dev/uinput` |
...
- Resolution/framerate: `RETROARCH_MODE` in `tesla-pi-retroarch-session` and
...
sudo rm -f /etc/systemd/system/cage-retroarch.service \
           /usr/local/sbin/tesla-pi-retroarch /usr/local/bin/tesla-pi-retroarch-session \
           /etc/sudoers.d/tesla-pi-retroarch /etc/udev/rules.d/99-tesla-pi-uinput.rules \
           /etc/modules-load.d/tesla-pi-retroarch.conf
sudo systemctl daemon-reload
```
The Node bridge goes inert once `/usr/local/sbin/tesla-pi-retroarch` is gone.
