...
**Where:** `mytesla/index.js` (WebSocketServer constructors + `broadcast` callsites)
...
**Where:** `mytesla/static/worker.js`
...
**Where:** `mytesla/static/index.html` (video `onMessage`)
...
### 4. Drop `network-online.target` from `mytesla.service`
**Where:** `mytesla/systemd/mytesla.service`
...
**Where:** `mytesla/index.js` (`restartCarplay`)
...
**Where:** `mytesla/static/index.html`
...
**Where:** `mytesla/conf/nginx-carplay.conf`
...
sudo systemctl restart mytesla
...
sudo systemd-analyze critical-chain mytesla.service
journalctl -u mytesla -b 0 -o short-monotonic | grep -E "Started|http_listen|carplay_started"
```
...
**Where:** `mytesla/index.js` `config.mediaDelay`
...
- **Auto-update from home Wi-Fi** — piggyback on `cert-renew-watch`'s "Pi is indoors" detection: while joined to home Wi-Fi, do `git fetch && git diff --quiet HEAD origin/main || (git pull && systemctl restart mytesla)`. Saves the manual ssh+pull cycle.
...
**Where:** `mytesla/index.js` `config.fps` and `mytesla/static/worker.js` `const fps`.
...
**Diagnostic:** `index.js` exposes a 1 Hz video frame/byte counter gated on `LOG_FPS=1`. Set it in `/etc/default/mytesla` (`LOG_FPS=1`) and `journalctl -u mytesla -f | grep video_rate` to re-measure if anything changes (different iOS version, new dongle firmware, lower-power Pi).
